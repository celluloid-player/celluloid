...
celluloid_header_bar_init(CelluloidHeaderBar *hdr)
{
	...
	gtk_menu_button_set_label(GTK_MENU_BUTTON(hdr->open_btn), _("Open"));
	gtk_menu_button_set_always_show_arrow(GTK_MENU_BUTTON(hdr->open_btn), TRUE);
	gtk_widget_set_tooltip_text(hdr->open_btn, _("Select a Media File, Folder or URL"));
	gtk_widget_set_tooltip_text(hdr->menu_btn, _("Main Menu"));
	...
	gchar css_data[] = ".top-bar .floating-header button {border-radius: 999px;}" ".floating-header windowcontrols image {background-color: rgba(0,0,0,0.65); color:var(--light-1);}" " headerbar.floating-header  {margin: 12px 12px 12px 12px;}";
...
celluloid_header_bar_set_floating(CelluloidHeaderBar *hdr, gboolean floating)
{
	...
	open_toggle_btn = gtk_widget_get_first_child(GTK_WIDGET(hdr->open_btn));
	menu_toggle_btn = gtk_widget_get_first_child(GTK_WIDGET(hdr->menu_btn));
	if(floating)
	{
		gtk_widget_add_css_class
			(GTK_WIDGET(hdr->header_bar), "floating-header");
		gtk_widget_add_css_class(GTK_WIDGET(hdr->fullscreen_btn), "osd");
		gtk_widget_add_css_class(GTK_WIDGET(hdr->fullscreen_btn), "raised");
		gtk_widget_add_css_class(GTK_WIDGET(open_toggle_btn), "osd");
		gtk_widget_add_css_class(GTK_WIDGET(open_toggle_btn), "raised");
		gtk_widget_add_css_class(GTK_WIDGET(menu_toggle_btn), "osd");
		gtk_widget_add_css_class(GTK_WIDGET(menu_toggle_btn), "raised");
		adw_header_bar_set_show_title(ADW_HEADER_BAR(hdr->header_bar), FALSE);
	}
	else
	{
		gtk_widget_remove_css_class
			(GTK_WIDGET(hdr->header_bar), "floating-header");
		gtk_widget_remove_css_class(GTK_WIDGET(hdr->fullscreen_btn), "osd");
		gtk_widget_remove_css_class(GTK_WIDGET(hdr->fullscreen_btn), "raised");
		gtk_widget_remove_css_class(GTK_WIDGET(open_toggle_btn), "osd");
		gtk_widget_remove_css_class(GTK_WIDGET(open_toggle_btn), "raised");
		gtk_widget_remove_css_class(GTK_WIDGET(menu_toggle_btn), "osd");
		gtk_widget_remove_css_class(GTK_WIDGET(menu_toggle_btn), "raised");
		adw_header_bar_set_show_title(ADW_HEADER_BAR(hdr->header_bar), TRUE);
	}
}
