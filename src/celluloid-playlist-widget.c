...
celluloid_playlist_widget_init(CelluloidPlaylistWidget *wgt)
{
 ...
 /*  	adw_header_bar_pack_end(ADW_HEADER_BAR(wgt->top_bar), wgt->select_button); */
	...
	gtk_search_entry_set_placeholder_text(GTK_SEARCH_ENTRY(wgt->search_entry), _("Search in playlist…"));
	...
	gtk_button_set_icon_name(GTK_BUTTON(wgt->loop1_btn), "media-playlist-repeat-song-symbolic");
	gtk_box_append(GTK_BOX(wgt->bottom_bar), wgt->loop1_btn);
	gtk_toggle_button_set_group(GTK_TOGGLE_BUTTON(wgt->loop1_btn), GTK_TOGGLE_BUTTON(wgt->playlist_consecutive));
	gtk_widget_set_tooltip_text(wgt->loop1_btn, _("Repeat file"));

	wgt->loop_list_btn = gtk_toggle_button_new();
	gtk_button_set_icon_name(GTK_BUTTON(wgt->loop_list_btn), "media-playlist-repeat-symbolic");
	gtk_box_append(GTK_BOX(wgt->bottom_bar), wgt->loop_list_btn);
	gtk_toggle_button_set_group(GTK_TOGGLE_BUTTON(wgt->loop_list_btn), GTK_TOGGLE_BUTTON(wgt->playlist_consecutive));
	gtk_widget_set_tooltip_text(wgt->loop_list_btn, _("Repeat playlist"));

	wgt->shuffle_btn = gtk_toggle_button_new();
	gtk_button_set_icon_name(GTK_BUTTON(wgt->shuffle_btn), "media-playlist-shuffle-symbolic");
	gtk_box_append(GTK_BOX(wgt->bottom_bar), wgt->shuffle_btn);
	gtk_widget_set_tooltip_text(wgt->shuffle_btn, _("Shuffle playlist"));
						...
						GPtrArray* playlist )
{
	...
	char *items_count;
	items_count =  g_strdup_printf (ngettext ("%ld file", "%ld files", wgt->playlist_count), wgt->playlist_count);
